...
```acceptance-report
{
  "criteriaSatisfied": [
    {
      "id": "criterion-1",
      "status": "satisfied",
      "evidence": "Concrete severity-ranked findings and actionable tasks name src/app/research.rs, src/ui/popups/research_live.rs, src/ui/history.rs, src/app/mod.rs, src/ui/mod.rs, and exact test files."
    }
  ],
  "changedFiles": [],
  "testsAddedOrUpdated": [],
  "commandsRun": [
    {
      "command": "rg -n ResearchUpdate|research_stage|ResearchLive src",
      "result": "passed",
      "summary": "Located research update producers, consumers, popup shortcut, and persistence paths."
    }
  ],
  "validationOutput": [
    "Reviewed current Searcher forwarding, Verifier event swallowing, label-based DB upsert, popup rendering, and history cache behavior."
  ],
  "residualRisks": [
    "Live provider behavior still requires manual end-to-end validation after implementation.",
    "Lifecycle state inferred from standardized text prefixes can drift if a future emitter bypasses the convention."
  ],
  "noStagedFiles": true,
  "diffSummary": "Planning-only review; no source files modified.",
  "reviewFindings": [
    "high: src/app/research.rs and src/ui/history.rs - in-place stage updates may remain stale because the transcript cache keys on message count.",
    "high: src/app/research.rs - Verifier tool/status activity is swallowed.",
    "medium: src/app/research.rs - Searcher rows lack terminal states and round-specific identity.",
    "medium: src/ui/popups/research_live.rs - activity is flat, clipped, non-navigable, and the view is poorly discoverable."
  ],
  "manualNotes": "Prefer migration-free standardized detail prefixes for the first iteration; preserve the plan approval gate and use an explicit Ctrl+Space hint rather than auto-opening before approval."
}
```
