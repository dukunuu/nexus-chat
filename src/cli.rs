...
fn skills_list() -> Result<()> {
    let space = space::Space::open()?;
    let skills = crate::skills::load_skills(&crate::skills::skills_dir(&space.root));
    if skills.is_empty() {
...
async fn skills_install(spec: &str) -> Result<()> {
    ...
    let space = space::Space::open()?;
    let dest = crate::skills::skills_dir(&space.root);
    let name =
        crate::skills::install_from_github(&reqwest::Client::new(), &owner, &repo, &path, &dest)
            .await?;
...
async fn update() -> Result<()> {
    match crate::update::latest_version().await {
        Some(latest) if crate::update::version_gt(&latest, crate::update::CURRENT) => {
            out(format!(
                "update available: v{latest} (you have {}) — run `cargo install nexus-chat`",
                crate::update::CURRENT
            ));
        }
        _ => out(format!("up to date — v{}", crate::update::CURRENT)),
    }
    Ok(())
}
