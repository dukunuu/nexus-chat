...
impl App {
    ...
    pub(crate) fn switch_space_cli(&mut self, name: &str) -> Result<()> {
        let row = self
            .db
            .list_spaces()
    ...
    pub(crate) async fn ask_headless(
        &mut self,
        prompt: String,
        opts: TurnOpts,
    ) -> Result<AskOutcome> {
    ...
    pub(crate) async fn chat_headless(&mut self, quiet: bool) -> Result<()> {
        if !self.backends.any() {
            bail!(
                "no API key configured — set one with /login in the TUI, or export \
                 $OPENROUTER_API_KEY / $OPENAI_API_KEY / $OPENCODE_API_KEY"
    ...
    pub(crate) async fn research_headless(
        &mut self,
        topic: String,
        approve: bool,
        opts: TurnOpts,
    ...
    pub(crate) async fn watch_run_headless(
        &mut self,
